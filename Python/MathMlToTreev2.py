...
mathMlSymbolsWithUndercoverMultiplySign=["[msqrt]", "[&alpha;]", "[&beta;]", "[&gamma;]", "[&delta;]", "[sin]", "[cos]", "[tan]", "[cot]", "[pi]", "[e]"]
    ...
    for i in range(len(inputString)):
        for keyValue in openingSymbols:
            if inputString[i:(i + len(keyValue))] == keyValue:
                if inputString[(i+len(keyValue)+1):(i+len(keyValue)+len(mathMlSymbolOpenRow)+1)] == mathMlSymbolOpenRow:
    ...
    for key in keyValue:
        ...
        while index < len(inputString):
            ...
            if inputString[(index+len(key)+1):indexClosingAfterSpecial+1]=="<mrow>":
                ...
                if key!="<msqrt>":
                    output=inputString[:indexOpeningAfterSpecial]+"<mrow>"+inputString[indexOpeningAfterSpecial:indexOfClosingAfterFirstClosing+1]+"</mrow><mrow>"+inputString[indexOfClosingAfterFirstClosing+1:(indexOfKeyClosing-1)]+"</mrow>"+inputString[(indexOfKeyClosing-1):]
                    inputString=output
                    index+=1
                else:
    ...
    for key in keyValue:
        ...
        while numberOfKeyOccurences>0:
            ...
            for k in inputString:
                ...
                kindex=kindex+1
                if (indexOfLeftBracket==indexOfRightBracket+1) and (numberOfLeftBrackets==numberOfRightBrackets+1):
                    output=inputString[:indexOfKeyValue]+inputString[indexOfKeyValue+len(key):indexOfRightBracket+1]+'['+key[1:(len(key)-1)]+']'+inputString[indexOfLeftBracket:]
            inputString = output
            numberOfKeyOccurences=numberOfKeyOccurences-1
